        ...
        >
          ...
          {error (
            <div className="bg-red-100 text-red-700 p-4 rounded-lg">
              ...
              <Button variant="outline" className="mt-2" tabIndex={0} role="button" onKeyDown={(e) => e.key === "Enter" && {() => window.location.reload()}>
                Retry
              </Button>
            </div>
          )}
          ...
          {!loading && !error && data.length === 0 (
            <div className="flex flex-col items-center justify-center h-96 text-center text-muted-foreground">
              ...
              <p className="text-sm mt-2">Try adding something or check your filters.</p>
              <Button className="mt-4" tabIndex={0} role="button" onKeyDown={(e) => e.key === "Enter" && {() => router.push("/create")}>Create New</Button>
            </div>
          )}
          ...
          {!loading && !error && data.map((item, i) => (
            ...
            >
              <Card className="p-4">
                ...
                <Button className="mt-2" tabIndex={0} role="button" onKeyDown={(e) => e.key === "Enter" && {() => router.push(`/detail/${item.id}`)}>
                  View
                </Button>
              </Card>
            </motion.article>
      ...
      <Dialog open={showOnboarding} onOpenChange={setShowOnboarding}>
        <DialogContent role="dialog" aria-labelledby="welcome-heading">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Start by comparing things you love. Your preferences will shape who you are.
          </p>
